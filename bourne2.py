...
def reset_bourne_protocol():
    ...
    # steps states
    st.session_state.bourne_1_done = False
    st.session_state.bourne_2_done = False
    st.session_state.bourne_3_done = False
    # step KPIs
    st.session_state.bourne_1_kpi = [None, None, None]
    st.session_state.bourne_2_kpi = [None, None, None]
    st.session_state.bourne_3_kpi = [None, None, None]

    # delete variables
    vars_del = ['bourne_1_conditions', 'bourne_2_conditions', 'bourne_3_conditions',
                'bourne_rpm', 'bourne_volume', 'bourne_feed',
                'bourne_mid_rpm', 'bourne_mid_volume']
    for var in vars_del:
        if var in st.session_state:
            # write var name to txt file for debugging
            with open("debug.txt", "a") as f:
                # get line number of code
                f.write(f"Deleting variable: {var}\n")
...
if 'bourne_1_done' not in st.session_state:
    st.session_state.bourne_1_done = False

if 'bourne_2_done' not in st.session_state:
    st.session_state.bourne_2_done = False

if 'bourne_3_done' not in st.session_state:
    st.session_state.bourne_3_done = False
...
# set default stirrer speed to midpoint of range
if 'bourne_rpm' not in st.session_state:
    st.session_state.bourne_rpm = [None, None, None]
    st.session_state.bourne_rpm[1] = (rpm_min + rpm_max) / 2

    # used for ensuring smooth update of variable
    st.session_state.bourne_mid_rpm = str(st.session_state.bourne_rpm[1])

# get reactor volume range
V_min = r[('Volume Min', 'L')]
V_max = r[('Volume Max', 'L')]

# default volume to midpoint of range
if 'bourne_volume' not in st.session_state:
    st.session_state.bourne_volume = [None, None, None]
    st.session_state.bourne_volume[1] = (V_min + V_max) / 2

    # used for ensuring smooth update of variable
    st.session_state.bourne_mid_volume = str(st.session_state.bourne_volume[1])

# *************** variable update functions ****************

def update_rpm():
    try:
        st.session_state.bourne_rpm[1] = float(st.session_state.bourne_mid_rpm)
    except ValueError:
        st.error("Invalid input for stir speed. Please enter a number.")

def update_vol():
    try:
        st.session_state.bourne_volume[1] = float(st.session_state.bourne_mid_volume)
    except ValueError:
        st.error("Invalid input for volume. Please enter a number.")

# ********************************************************

# get centerpoint conditions; don't allow changes once step is done to preserve results
if not st.session_state.bourne_1_done:

    col1.text_input(
    "Centerpoint Stir Speed [rpm]",
    key="bourne_mid_rpm",
    on_change=update_rpm
    )

    col2.text_input(
    "Centerpoint Volume [L]",
    key="bourne_mid_volume",
    on_change=update_vol
    )

PV_factor = 10

# Calculate rpm values based on 100x P/V changes
st.session_state.bourne_rpm[0] = round((1/PV_factor * st.session_state.bourne_rpm[1]**3)**(1/3), 0)
st.session_state.bourne_rpm[2] = round((PV_factor * st.session_state.bourne_rpm[1]**3)**(1/3), 0)

# create dict of conditions and KPIs
if 'bourne_1_kpi' not in st.session_state:
     st.session_state.bourne_1_kpi = [None, None, None]

conditions = {'Volume [L]': [st.session_state.bourne_volume[1],
                             st.session_state.bourne_volume[1],
                             st.session_state.bourne_volume[1]],
            'Agitation [rpm]': st.session_state.bourne_rpm,
             'KPI': st.session_state.bourne_1_kpi}
...
def bourne_1_update():
    ...
    try:
        ...
        # preserve KPI values
        st.session_state.bourne_1_kpi = [kpi_low, kpi_mid, kpi_high]
...
if 'feed_mid' not in st.session_state:
    st.session_state.feed_mid = 0.1
feed_mid = st.session_state.feed_mid

# get centerpoint conditions
if not st.session_state.bourne_2_done:
...
rpm_mid = st.session_state.bourne_rpm[1]
V_mid = st.session_state.bourne_volume[1]

# create dict of conditions and KPIs
if 'bourne_2_kpi' not in st.session_state:
     st.session_state.bourne_2_kpi = [None, None, None]
feed_conditions = {'Feed Rate [kg/h]': [feed_low, feed_mid, feed_high],
            ...
            'KPI': st.session_state.bourne_2_kpi}
...
def bourne_2_update():
    ...
    # preserve KPI values
    st.session_state.bourne_2_kpi = [kpi_low, kpi_mid, kpi_high]
    ...
    st.session_state.bourne_2_done = True

# ensure table becomes immutable once step is done to preserve results
if st.session_state.bourne_2_done:
...
if (not st.session_state.bourne_1_done) or (st.session_state.bourne_2_done):
...
if 'bourne_3_kpi' not in st.session_state:
     st.session_state.bourne_3_kpi = [None, None, None]

feed_location = {'Agitation [rpm]': st.session_state.bourne_rpm,
                'Volume [L]': [st.session_state.bourne_volume[1],
                               st.session_state.bourne_volume[1],
                               st.session_state.bourne_volume[1]],
            'KPI': st.session_state.bourne_3_kpi}
...
def bourne_3_update():
    ...
    # preserve KPI values
    st.session_state.bourne_3_kpi = [kpi_low, kpi_mid, kpi_high]
    ...
    st.session_state.bourne_3_done = True

# ensure table becomes immutable once step is done to preserve results
if st.session_state.bourne_3_done:
...
if (not st.session_state.bourne_2_done) or (st.session_state.bourne_3_done):
